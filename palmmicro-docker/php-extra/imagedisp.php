...
function ImgAutoQuote($strPathName, $strText = '', $bChinese = true)
{
	...
	$strRootPathName = UrlModifyRootFileName($strPathName);
	if (!file_exists($strRootPathName))	return GetImgQuote($strPathName, $strText, $bChinese);
	$imgOrg = imagecreatefromjpeg($strRootPathName);
	if ($imgOrg === false)	return GetImgQuote($strPathName, $strText, $bChinese);
